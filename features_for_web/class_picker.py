...
def make_weekly_plan(days: List[str], groups: List[str], weight_kg: float, weekly_target_kcal: float,
    ...
    out["Day"] = out["Ngày"].map(DAY_VN_TO_EN)
    EN_ORDER = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    out["Day"] = pd.Categorical(out["Day"], categories=EN_ORDER, ordered=True)
    out = out.sort_values("Day")

    out = out.rename(columns={
        "Hoạt động": "Activity",
        "Thời gian (phút)": "Duration (min)",
        "Kcal ước tính": "Estimated kcal"
    })

    # Đưa cột Day lên đầu
    out = out[["Day", "Activity", "Duration (min)", "Estimated kcal"]]

    total = out["Estimated kcal"].sum()
    print(f"Total estimated kcal = {total:.1f} (Target = {weekly_target_kcal:.1f}) | Deviation = {total - weekly_target_kcal:+.1f}")
    return out
